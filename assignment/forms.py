...
class NewAssignmentForm(forms.ModelForm):
	...
	files = forms.FileField(widget=forms.FileInput(attrs={'single': True}), required=False)
...
class NewSubmissionForm(forms.ModelForm):
	file = forms.FileField(widget=forms.FileInput(attrs={'single': True}), required=True)
